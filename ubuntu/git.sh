...
generate_ssh_keys() {
    ...
    local -r EMAIL_ADDRESS="$(get_answer)"
    ssh-keygen -t rsa -b 4096 -C "$EMAIL_ADDRESS" -f "$1"
...
set_global_configs() {
    ...
    ask "Please provide a fullname: " && printf "\n"
    local -r FULLNAME="$(get_answer)"
